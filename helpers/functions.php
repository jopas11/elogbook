...
function parseImages($val) {
    ...
    return $trimmed !== '' ? array($trimmed) : array();
}

function uploadMultipleImages($files, $maxFiles = 5) {
    ...
    } elseif (!empty($files['name']) && $files['error'] === UPLOAD_ERR_OK) {
        $fileCount = 1;
    }

    if ($fileCount > $maxFiles) {
        flash('error', 'Maksimal ' . $maxFiles . ' foto.');
    ...
    for ($i = 0; $i < $count; $i++) {
        ...
        if ($convertedPath) {
            ...
            flash('error', 'Gagal menyimpan salah satu foto.');
            return '__FLASH_SET__';
        }
    }
