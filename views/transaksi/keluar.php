...
<div x-data="sparepartKeluarForm()" class="page-enter max-w-3xl mx-auto">
    ...
    <div class="flex gap-1 mb-6 bg-gray-100 dark:bg-gray-800 p-1 rounded-xl w-fit border border-gray-200 dark:border-gray-700">
        <button @click="tab = 'aset'; resetForm()"
                :class="tab === 'aset' ? 'bg-white dark:bg-gray-700 text-cyan-500 dark:text-cyan-400 shadow-sm' : 'text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-300'"
        ...
        <button @click="tab = 'nonaset'; resetForm()"
                :class="tab === 'nonaset' ? 'bg-white dark:bg-gray-700 text-cyan-500 dark:text-cyan-400 shadow-sm' : 'text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-300'"
                class="px-5 py-2 rounded-lg text-sm font-semibold transition-all duration-200 inline-flex items-center gap-2">
    ...
    <div class="flex justify-between items-center mb-6">
        <h2 class="text-2xl font-bold text-gray-800 dark:text-white gradient-text">
            Catat Sparepart Keluar <span class="text-cyan-500 dark:text-cyan-400">—</span>
            <span x-text="tab === 'aset' ? 'Aset' : 'Non-Aset'" class="text-cyan-500 dark:text-cyan-400"></span>
        </h2>
    </div>
        ...
        <form method="POST" action="index.php?url=sparepart_keluar" class="space-y-5" enctype="multipart/form-data">
            ...
            <template x-if="tab === 'aset'">
                <div class="space-y-5">
                    ...
                    <div class="bg-cyan-50 dark:bg-cyan-900/20 rounded-lg p-4 border border-cyan-100 dark:border-cyan-800/30">
                        ...
                        <div class="flex gap-2 relative">
                            <div class="flex flex-1">
                                ...
                                <input type="text" x-model="snAset" @input="cariBySn" @focus="showSuggestions = false; setTimeout(() => _fetchSuggestions(), 100)"
                                       class="flex-1 px-3 py-2.5 border border-gray-200 dark:border-gray-600 bg-gray-50 dark:bg-gray-700/50 dark:text-gray-200 rounded-r-xl text-base focus:ring-2 focus:ring-cyan-500/30 focus:border-cyan-500/50 outline-none transition-all duration-200 font-mono"
                                       placeholder="Masukkan serial number" autocomplete="off">
                            </div>
                            <button type="button" @click="cariBySn" :disabled="!snAset.trim()"
                                    class="px-4 py-2 bg-gradient-to-r from-cyan-500 to-violet-500 text-white rounded-lg text-sm font-medium hover:from-cyan-400 hover:to-violet-400 transition disabled:opacity-50 inline-flex items-center gap-1.5 magnetic-btn">
                            ...
                            <div x-show="showSuggestions" x-cloak
                                ...
                                <template x-for="(item, idx) in suggestions" :key="idx">
                                    <button type="button" @click="selectSuggestion(item)"
                                            class="w-full text-left px-3 py-2 text-sm hover:bg-cyan-50 dark:hover:bg-cyan-900/30 border-b border-gray-100 dark:border-gray-600 last:border-b-0 transition flex items-center gap-2">
                        ...
                        <div class="flex items-start gap-4">
                            <div class="flex-1">
                                <input type="file" name="image" accept="image/jpeg,image/png,image/webp" required
                                       ...
                                       class="w-full text-sm text-gray-500 dark:text-gray-400 file:mr-3 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-semibold file:bg-cyan-50 file:text-cyan-700 hover:file:bg-cyan-100 dark:file:bg-cyan-900/30 dark:file:text-cyan-400 dark:hover:file:bg-cyan-900/50 transition">
                            </div>
            ...
            <div class="flex justify-end gap-3 pt-2 border-t border-gray-200 dark:border-gray-700">
                ...
                <button type="submit"
                        :disabled="(tab === 'aset' && (!snFound || !sparepartId || !fotoPreviewAset)) || (tab === 'nonaset' && !fotoPreviewNonaset) || !!fotoError"
                        class="px-5 py-2.5 bg-gradient-to-r from-cyan-500 to-violet-500 text-white rounded-lg text-sm font-medium hover:from-cyan-400 hover:to-violet-400 transition-all inline-flex items-center gap-2 shadow-md shadow-cyan-500/20 disabled:opacity-50 disabled:cursor-not-allowed magnetic-btn">
