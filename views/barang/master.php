            ...
            darkSwal({
                ...
                html: `
                    <div class="text-left space-y-2 text-sm">
                        ...
                        ${(() => { var imgs = imgList(data.data.image); if (imgs.length) { return '<div class="mt-3"><strong>Foto:</strong><div class="flex flex-wrap gap-2 mt-1">' + imgs.map(function(p){ return '<img src="<?= APP_BASE_PATH ?>/' + escapeHtml(p) + '" class="w-24 h-24 rounded-lg border border-gray-200 dark:border-gray-600 object-cover cursor-pointer hover:opacity-80 transition" onclick="window.open(this.src)" loading="lazy">'; }).join('') + '</div></div>'; } return ''; })()}
                        ${logsHtml}
                    </div>
                `,
